...
from utils import ProgressMeter, AverageMeter, get_accuracy, compute_mean_std,get_three_source_loader, get_four_train_test_loader
...
def main(args, experiment=None):
    ...
    multi_domains_train_iter, test_loader = get_four_train_test_loader(args=args)
    ...
    if args.phase != 'train':
        print(f"--test phase--: use model path = {logger.get_checkpoint_path('best')}")
    ...
    for epoch in range(args.epochs):
        ...
        cls_losses_source, cls_source, f1_source, domain_dis, intra_dis, all_loss  = train_transfer(train_source_iter=multi_domains_train_iter,
                                                                              model=classifier,
                                                                              optimizer=optimizer,
...
def train_transfer(train_source_iter, model, optimizer, epoch, jmmd_loss, intra_loss_con, args):
    """
    ...
    for batch_idx in range(args.max_iter):
        ...
        texts_s4, imgs_s4, labels_s4 = next(train_source_iter[3])
        ...
        texts_s4 = texts_s4.to(device)
        imgs_s4 = imgs_s4.to(device)
        ...
        ft_s4, fv_s4, y_s4, instance_s4 = model(train_texts=texts_s4, train_imgs=imgs_s4)
        # ft_t, fv_t, y_t, instance_t = model(train_texts=texts_t, train_imgs=imgs_t)

        ft_s = torch.cat([ft_s1, ft_s2, ft_s3, ft_s4], dim=0)
        fv_s = torch.cat([fv_s1, fv_s2, fv_s3, fv_s4], dim=0)
        labels_s = torch.cat([labels_s1, labels_s2, labels_s3, labels_s4], dim=0)
        instances = torch.cat([instance_s1, instance_s2, instance_s3, instance_s4], dim=0)
        ...
        if args.lambda1 == 0:
            ...
            if args.da:
                if args.yadapt:
                    ...
                    y_s4 = F.softmax(y_s4, dim=1)
                    # y_t = F.softmax(y_t, dim=1)
                    transfer_loss = (jmmd_loss((ft_s1, fv_s1, y_s1), (ft_s4, fv_s4, y_s4)) + jmmd_loss((ft_s2, fv_s2, y_s2),
                    (ft_s4, fv_s4, y_s4)) + jmmd_loss((ft_s3, fv_s3, y_s3), (ft_s4, fv_s4, y_s4)) +
                                     jmmd_loss((ft_s1, fv_s1, y_s1), (ft_s2, fv_s2, y_s2)) + jmmd_loss((ft_s1, fv_s1, y_s1),
                                    (ft_s3, fv_s3, y_s3)) +
                                     jmmd_loss((ft_s2, fv_s2, y_s2), (ft_s3, fv_s3, y_s3))) / 6
                else:
                    transfer_loss = (jmmd_loss((ft_s1, fv_s1), (ft_s4, fv_s4)) + jmmd_loss((ft_s2, fv_s2),
                                                                                         (ft_s4, fv_s4)) + jmmd_loss(
                        (ft_s3,
                         fv_s3), (ft_s4, fv_s4)))/3 + (jmmd_loss((ft_s1, fv_s1), (ft_s2, fv_s2)) + jmmd_loss((ft_s1, fv_s1),
                                                                                                       (ft_s3, fv_s3)) +
        ...
        y_s = torch.cat([y_s1, y_s2, y_s3, y_s4], dim=0)
...
if __name__ == '__main__':
    ...
    # experiment.set_name(str(args.data) + '-' + str(args.tag) + '-' + str(args.epochs) + '-freeze8')
    experiment.set_name(str(args.data) + '-' + str(args.tag))

    experiment.add_tag(args.phase + '_mode')
    ...
    for seed_random in seed:
        ...
        acc_seeds.append(acc_seed) # shape = [3, 4] = [seeds_num, domains_num]
