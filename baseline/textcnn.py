    ...
    def load_embeddings(self):
        if self.mode == 'rand':
            if self.dataset_name == "Pheme":
                path_saved = "./dataset/vocab/pheme_vocab.pt"
            elif self.dataset_name == "Twitter":
                path_saved = "./dataset/vocab/twitter_vocab.pt"
            else:
            ...
            if mode[0] == 'roberta':
                ...
                config = AutoConfig.from_pretrained(os.path.join(path_saved, "chinese-roberta-wwm"))
                chinese_roberta = AutoModel.from_pretrained(os.path.join(path_saved, "chinese-roberta-wwm"), config=config)
                weight = chinese_roberta.get_input_embeddings().weight
                self.vocab_size = weight.shape[0]
                self.embedding_dim = weight.shape[1]
                self.embedding = nn.Embedding(self.vocab_size, self.embedding_dim).from_pretrained(
                    weight)
                # self.embedding.weight.data.copy_(roberta.get_input_embeddings().weight)
                del chinese_roberta, config, weight
