...
def get_embedding(model, loader, device, num_classes, args, with_emb=False, emb_dim=512):
	...
	with torch.no_grad():
		for batch_idx, (text_data, img_data, target) in enumerate(loader):
			text_data, img_data, target = text_data.to(device), img_data.to(device), target.to(device)
			if with_emb:
				_, _, e1, _, e2 = model(text_data, img_data, return_feature=True)
				embedding_pen[batch_idx*batch_sz:batch_idx*batch_sz + min(batch_sz, e2.shape[0]), :] = e2.cpu()
			else:
				_, _, e1 = model(text_data, img_data)
