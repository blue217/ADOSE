...
class MMESolver(BaseSolver):
	...
	def solve(self, epoch):
		...
		joint_loader = zip(self.src_loader, self.tgt_unsup_loader)
		for batch_idx, ((text_data_s, img_data_s, label_s), (text_data_tu, img_data_tu, _)) in enumerate(joint_loader):
			text_data_s, img_data_s, label_s = text_data_s.to(self.device), img_data_s.to(self.device), label_s.to(self.device)
			text_data_tu, img_data_tu = text_data_tu.to(self.device), img_data_tu.to(self.device)
			
			if self.da_round > 0:
				try:
					text_data_ts, img_data_ts, label_ts = next(tgt_sup_iter)
					text_data_ts, img_data_ts, label_ts = text_data_ts.to(self.device), img_data_ts.to(self.device), label_ts.to(self.device)
				except: break
			...
			# extract features
			_, _, score_s, _ = self.net(text_data_s, img_data_s)
			xeloss_src = src_sup_wt * nn.CrossEntropyLoss()(score_s, label_s)
			...
			xeloss_tgt = 0
			if self.da_round > 0:
				_, _, score_ts, _ = self.net(text_data_ts, img_data_ts)
				xeloss_tgt = nn.CrossEntropyLoss()(score_ts, label_ts)
				info_str += " Tgt Sup loss: {:.3f}".format(xeloss_tgt.item())
			...
			_, _, score_tu, _ = self.net(text_data_tu, img_data_tu, reverse_grad=True)
