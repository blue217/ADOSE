...
from utils import ProgressMeter, AverageMeter, get_accuracy, compute_mean_std, get_source_target_test_loader
...
def main(args, experiment=None):
    ...
    multi_source_iter, train_target_iter, label_target_iter, test_loader = get_source_target_test_loader(args=args)
    ...
    for epoch in range(args.epochs):
        ...
        cls_losses_source, cls_source, f1_source, domain_dis, intra_dis, all_loss  = train_transfer(
            train_source_iter=multi_source_iter, train_target_iter=train_target_iter, label_target_iter=label_target_iter,
            model=classifier, optimizer=optimizer, epoch=epoch, jmmd_loss=jmmd_loss, intra_loss_con=intra_loss, args=args)
...
def train_transfer(train_source_iter, train_target_iter, label_target_iter, model, optimizer, epoch, jmmd_loss, intra_loss_con, args):
    """
    ...
    for batch_idx in range(args.max_iter):
        ...
        texts_t_label, imgs_t_label, labels_t_label = next(label_target_iter)
        ...
        labels_t_label = labels_t_label.to(device)
        texts_t_label = texts_t_label.to(device)
        imgs_t_label = imgs_t_label.to(device)
        ...
        ft_tl, fv_tl, y_tl, instance_tl = model(train_texts=texts_t_label, train_imgs=imgs_t_label)

        ft_s = torch.cat([ft_s1, ft_s2, ft_s3, ft_tl], dim=0)
        fv_s = torch.cat([fv_s1, fv_s2, fv_s3, fv_tl], dim=0)
        labels_s = torch.cat([labels_s1, labels_s2, labels_s3, labels_t_label], dim=0)
        instances = torch.cat([instance_s1, instance_s2, instance_s3, instance_tl], dim=0)
        ...
        y_s = torch.cat([y_s1, y_s2, y_s3, y_tl], dim=0)
...
if __name__ == '__main__':
    ...
    experiment = Experiment(
        ...
        project_name="fake-news-semisupervise",
        workspace="",
    )
