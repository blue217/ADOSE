...
from model.classify_model import JointModel_TextCnn_abla, DomainDiscriminator, grad_reverse
...
def get_parser():
    ...
    parser.add_argument("--active_type", type=str, default='abla_model',
...
def train(args, task):
    ...
    model = JointModel_TextCnn_abla(out_size=256, num_label=2, freeze_id=args.freeze_resnet,
    ...
    for epoch in range(1, args.train_epochs + 1):
        ...
        for batch_idx in range(args.max_iter):
            ...
            y_fuse = torch.cat([logits_s1, logits_s2, logits_s3], dim=0)
            ...
            if len(tgt_select_ds) > 0:
                ...
                y_fuse = torch.cat([y_fuse, logits_tl], dim=0)
            ...
            final_ce_loss = F.cross_entropy(y_fuse, labels)
