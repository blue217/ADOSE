...
class PhemeSet(Dataset):
    ...
    def add_item(self, addition):
        for sample in addition:
            self.dataset.append(sample)

    def remove_item(self, idx_list):
        idx_list.sort(reverse=True)
        removed_items = []  # 用于存储删除的样本

        for idx in idx_list:
            if 0 <= idx < len(self.dataset):  # 确保索引在合法范围内
                removed_items.append(self.dataset.pop(idx))  # 删除并保存被删除的样本
...
class WeiboSet(Dataset):
    def __init__(self, json_path="./weibo.json", img_path="./images",
                 type=0, events=None, visual_type='resnet', stage='train'):
        """
        Args:
            json_path:
        ...
        if self.visual_type == "resnet":
            # self.tfms = transforms.Compose([transforms.Resize(256),
