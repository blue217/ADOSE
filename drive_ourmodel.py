...
def get_parser():
    ...
    parser.add_argument('-d', '--data', metavar='DATA', default='Pheme', choices=['Pheme', 'Weibo', 'Cross'],
    ...
    parser.add_argument('--da', default='false', type=str,
                        help='whether to apply domain adaptation')
    parser.add_argument('--lambda1', default=0.1, type=str,
    ...
    parser.add_argument('--lambda2', default=0.5, type=str,
...
def main(args, experiment=None):
    ...
    multi_source_domains_loader, multi_source_domains_iter, train_target_loader, train_target_iter, test_loader = (
        get_three_source_loader(args=args))
    ...
    for epoch in range(args.epochs):
        ...
        cls_loss_target, acc1, f1_fake, f1_real, recall_target, pre_target = validate(test_loader, classifier, device)
        ...
        f1_store.append([f1_fake, f1_real])
    ...
    # acc1_store.sort()   # len = epochs, each value corresponds to target domain acc
    # f1_store.sort()
    max_index = acc1_store.index(max(acc1_store))
    ...
    # return acc1_store[-1], f1_store[-1]
    return acc1_store[max_index], f1_store[max_index][0], f1_store[max_index][1]
...
def validate(val_loader, model, device):
    ...
    f1_fake_scores = AverageMeter('F1_fake', ':3.4f')  # 标签1 (fake)
    f1_real_scores = AverageMeter('F1_real', ':3.4f')  # 标签0 (real)
    recall_scores = AverageMeter('Recall_score', ':3.4f')
    precession_scores = AverageMeter('Precession_score', ':3.4f')
    progress = ProgressMeter(
        len(val_loader),
        [batch_time, losses, top1, f1_fake_scores, f1_real_scores, recall_scores, precession_scores], prefix='Test: ')
    ...
    with torch.no_grad():
        ...
        # 将列表转为numpy数组
        real_label_array = np.array(real_label_list)
        predicted_y_array = np.array(predicted_y_list)

        # 计算两个类别的F1值
        # 使用 pos_label 参数分别指定正例为1和0
        f1_fake = metrics.f1_score(real_label_array, predicted_y_array, pos_label=1)  # 标签1 (fake)
        f1_real = metrics.f1_score(real_label_array, predicted_y_array, pos_label=0)  # 标签0 (real)
        ...
        # f1_scores.update(float(f1), 1)
        f1_fake_scores.update(float(f1_fake), 1)
        f1_real_scores.update(float(f1_real), 1)
    ...
    return losses.avg, top1.avg, f1_fake_scores.avg, f1_real_scores.avg, recall_scores.avg, precession_scores.avg
...
if __name__ == '__main__':
    ...
    if args.data == "Pheme":
        ...
        source = ["society", "entertainment", "education", "health"]
        log_dir = ["edh2s", "sdh2e", "seh2d", "sed2h"]
    # experiment = None

    experiment = Experiment(
        api_key="H7m2z8rBavYpcKODTLJg74F3v",
        project_name="Fake-News-ADA",
        workspace="",
    )
    ...
    experiment.set_name(str(args.data) + '-' + str(args.tag) + '-' + str(args.epochs))
    experiment.add_tag(args.phase)
    ...
    for seed_random in seed:
        ...
        for i, target_domain in enumerate(source):
            ...
            acc, f1_fake, f1_real = main(args, experiment)
            acc_seed.append(acc)
            f1_seed.append([f1_fake, f1_real])


        acc_seeds.append(acc_seed) # shape = [1, 4] = [seeds_num, domains_num]
        f1_seeds.append(f1_seed) # shape = [1, 4, 2]
    ...
    for i, domain in enumerate(source):
        ...
        event_metric[domain] = [acc_seeds[0][i], f1_seeds[0][i][0], f1_seeds[0][i][1]]
    # print(event_metric)
